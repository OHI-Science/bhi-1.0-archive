...
# 11. BSEP nutrient data exploration
...
winter_data_raw <- read.csv(file.path(dir_secchi, 'BSEP_nutrient_data/Nutrients_winter_1980-2016_0-10m_id_assigned.csv'))
summer_data_raw <- read.csv(file.path(dir_secchi, 'BSEP_nutrient_data/Summer_1980-2016_chl_0-10m_id_assigned.csv'))
...
nut_basin_level <- full_join(nut_combined, bhi_basin_lookup, by = "Basin") %>% 
  filter(!is.na(BHI_ID))

####### add reference point ##########
ref_data <- read.csv(file.path(dir_secchi, 'BSEP_nutrient_data/BSEP_nutrient_reference.csv')) %>% 
...
nut_data_with_ref <- full_join(nut_combined, ref)
...
nut_status_all_years <- nut_data_with_ref %>% 
  mutate(dip_status = pmin(1, 1/(dip_ave / dip_ref)), 
         din_status = pmin(1, 1/(din_ave / din_ref)),
         chla_status = pmin(1, 1/(chla_ave / chla_ref))) %>% 
  group_by(BHI_ID, year) %>% 
  mutate(nut_status = geometric.mean(c(dip_status, din_status, chla_status)), 
         nut_status = as.numeric(str_replace_all(nut_status, "NaN", ""))) %>% 
  select(BHI_ID, year, dip_status, din_status, chla_status, nut_status)

# choose status year 
## 2012 as it has relatively most complete data for all regions 
nut_status <- nut_status_all_years %>% 
  filter(year == 2012)
