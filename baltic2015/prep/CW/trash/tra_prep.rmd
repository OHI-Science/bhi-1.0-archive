...
modeled_waste_2015 = baltic_data %>% 
  dplyr::select(country, modeled_waste_2010, modeled_waste_2025) %>% 
...
tra_value_data= modeled_waste_2015%>%
...
data_clean_select = data_clean %>%
  dplyr::select(country, coastal_pop, modeled_waste_2010_orig, modeled_waste_2025_orig)
...
2015: Using modeled 2015 data for status but use max 2010 as a reference point. 

```{r explore status 2015}

## scores by country
scores_by_country = modeled_waste_2015 %>% 
    filter(country %in% baltic$country) %>%  # choosing only Baltic countries
