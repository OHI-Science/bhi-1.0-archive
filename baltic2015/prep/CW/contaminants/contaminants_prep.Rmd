...
library('ggmap')
  map = get_map(location = c(8.5, 53, 32, 67.5))
    ...
    map_data1 = dioxin_sumteq2 %>%
                select(lat,lon)%>%
                distinct()

    ##set up the plot
    plot_map1 = ggmap(map) +
      geom_point(aes(x=lon, y=lat, colour="black"), data=map_data1,size = 2.5)
...
map = get_map(location = c(8.5, 53, 32, 67.5))
    ...
    ##plot the map
    plot_map1  +
      ggtitle('PFOS herring sampling locations') +
...
pfos3 = pfos2 %>%
        select(country,station, lat, lon, bhi_id, basin, date, year, sub_samp_ref,pfos_tissue_original_type,
               variable,value) %>%
        filter(!is.na(value))
...
pfos4 = pfos3 %>%
        select(-sub_samp_ref)%>%
        group_by(country,station,lat,lon,bhi_id,basin,date,year,pfos_tissue_original_type, variable)%>%
...
### 6.5 Basin means and status calculation
...
pfos6 = pfos5 %>%
        select(country,bhi_id,basin,variable,mean_date_loc) %>%
        group_by(country,bhi_id,basin,variable)%>%
