...
TODO: there is one parameter in the BHI_MPA data set "Shape intersecting the two data files created 184 duplicates of the 42 regions... I removed them for now, but I need to dig deeper to find out why that happene and if that in any way affected the accuracy of the data. 
...
# duplicates = mpa_bhi_data[duplicated(mpa_bhi_data[,c('Shape_area', 'Date_Est', 'rgn_key', 'Subbasin', 'HELCOM_ID', 'BHI_ID', 'Area_km2')]),] # revealed 184 duplicates
# unique(mpa_bhi_data[,c('rgn_key', 'Subbasin', 'HELCOM_ID', 'BHI_ID', 'Area_km2')]) %>%
mpa_area_per_rgn = mpa_bhi_data %>%
                   dplyr::select(BHI_ID, 
                                 AreaTot_Ha, 
                                 AreaMar_Ha, 
                                 Shape_area, # what's the difference among the three areas?
                                 Date_Est, 
                                 MPA_Status) %>%
                   group_by(BHI_ID) %>%
                   summarize(mpa_area = sum(AreaTot_Ha)) # or should we use the other two areas?
