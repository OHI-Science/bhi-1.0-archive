...
# count the number of "no matchs" from mgmt_status_old to mpa)area_per_rgn
...
mpa_status_new = read.csv(file.path(dir_prep, "mpa_data_database/MPA_status_download06June2016.csv"), sep = ";") %>%
              mutate(status = str_replace_all(Status, " ", "_")) %>%
              dplyr::select(name = HELCOM_MPA_name,
                        
                            status_csv = status) 

# combine mpa_status with shape file data

mpa_mgmt_new = right_join(mgmt_status, mpa_area_per_rgn,  # select only MPAs with BHI_ID
                      by = c("BSPA_ID"))
           # mutate(mgmt_status = ifelse(is.na(mgmt_status), as.character(mpa_status), mgmt_status),
           #        date_est = str_sub(date_est, start = -4)) %>% # only select the year of establishment
           # dplyr::select(-mpa_status)
