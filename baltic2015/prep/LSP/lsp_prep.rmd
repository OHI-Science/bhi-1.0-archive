...
There are four types of management status and their corresponding weight: Desginated (0.3), Managed (0.6), Indevelopment (0.6), and Implemented (1).  
...
## to check the number of unique MPAs
...
num_mismatch = dplyr::count(mpa_mgmt_old, !mgmt_status== "NA") # 30 no match
...
mgmt_weight = data.frame( mgmt_status = c("Designated", "In development", "Implemented"),
                          weight = c(0.3, 0.6, 1) )

mpa_mgmt = full_join(mpa_mgmt, mgmt_weight) %>%
           group_by(name) %>%
...
# count number of unique mpa names in new mpa mgmt status csv 

mpa_name = dplyr::select(mgmt_status_new, name_csv)
unique_mpa_csv_new = base::unique(mpa_name) ## total of 175 names that are unique, 175 rows. 

# combine mpa_status with shape file data

mpa_mgmt_new = right_join(mgmt_status_new, mpa_area_per_rgn,  # select only MPAs with BHI_ID
                          by = "BSPA_ID")

## got this Error: data frame column with incompatible number of rows (214), expecting : 226
## could not join the two files. Could be because 
...
# count number of MPAs in shape file with no matches in csv file. 
num_mismatch_new = dplyr::count(mpa_mgmt_new, !mgmt_status== "NA") # 30 no match
