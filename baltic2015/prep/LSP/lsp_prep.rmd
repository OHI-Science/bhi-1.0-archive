...
#                  dplyr::select(BSPA_ID, BHI_ID, country, eez_area_km2, mpa_area_km2, weight, year = date_est) %>%
#                  filter(!duplicated(BSPA_ID)) %>% #remove duplicated rows of the same MPA
#                  group_by(country) %>%
#                  arrange(year) %>%
#                  mutate(total_eez_km2 = sum(eez_area_km2)) %>%
#                  summarize(cum_mpa_area = cumsum(mpa_area_km2)) %>%
#                  ungroup %>%
#                  group_by(country, year) %>%
#                  mutate(sum_wt_mpa_area = sum(cum_mpa_area * weight)),
...
## new info: 
### calculating total eez area, by BHI_ID
...
### calculating cumulative mpa area, by country, by year -> 
### TODO: should do sum(area*wt) first, then cumsum!!
cum_mpa = mpa_mgmt_with_wt %>%
                 dplyr::select(BSPA_ID, BHI_ID, country, eez_area_km2, mpa_area_km2, year = date_est) %>%
                 filter(!is.na(year), 
                        !duplicated(BSPA_ID)) %>% # the same MPA could exist in different BHI_IDs 
                 group_by(country, year) %>%
                 dplyr::summarize(sum_mpa_km2 = sum(mpa_area_km2)) %>%
                 arrange(year) %>%
                 mutate(cum_mpa_area = cumsum(sum_mpa_km2)) %>%
                 dplyr::select(-sum_mpa_km2)

### cobmine cumulative mpa area with total eez area

status_per_year_by_country = full_join(cum_mpa, eez_data, by = 'country') %>% 
  full_join(dplyr::select(mpa_mgmt_with_wt,
                          - mpa_area_km2,
                          - eez_area_km2),
                          by = 'country') %>%
  filter(!is.na(year)) %>%
  group_by(country, year) %>%
  summarize(status = cum_mpa_area*weight/total_eez_km2) 
  

## previous trend_data_3 attempts
