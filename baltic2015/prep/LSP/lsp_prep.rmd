...
w_i are based upon management status, must be managed to count  
0 = designated  
0.6 = designated and partly managed  
1.0 designated and managed  
...
NING - PLEASE EXPLAIN METHODS HERE


##### 3.2.2 Alternative 2 - Cumulative area
...
##### 3.2.3 Alternative 3 - Trend of status.
NING - PLEASE EXPLAIN METHODS HERE  
...
TO DO..........Ning can you calculate status as described in Section 3.1.2
1. Calculate Status  
2. Plot status by country  
...
status_by_BHI_ID_plot <- ggplot(r.status_2, aes(x = rgn_id, y = score)) +
 geom_bar(stat = 'identity') +
 theme(axis.text.x = element_text(angle = 75, hjust = 1)) + 
 labs(title = 'LSP status by BHI ID',
      x = 'BHI ID', 
...
TO DO..........Ning can you calculate status as described in Section 3.1.3

1. Calculate Status  
2. Plot status components separately by country  
2. Plot status by country  
...
cum_mpa_area_wt_3 = mpa_mgmt_with_wt_2 %>%
                 dplyr::select(BSPA_ID, BHI_ID, weight, country, mpa_area_km2, year = date_est) %>%
                 filter(!is.na(year), 
                        !duplicated(BSPA_ID)) %>% # the same MPA could exist in different BHI_IDs 
                 group_by(country, year) %>%
...
status_per_year_by_country_3 = full_join(cum_mpa_area_wt_3, eez_data, by = 'country') %>% 
  group_by(country, year) %>%
  summarize(status = round(max( -1, min(1, cum_mpa_wt/ref_eez_km2)) * 100, 1)) %>%
  ungroup() %>%
  arrange(country, year)

## Calculate: Xlsp_country_mpa_area_score = pmin(1,MPA_area_country / area_10percent_eez_country)
 
Xlsp_country_mpa_area_score = mpa_mgmt_with_wt %>%
...
Xlsp_country_status =  Xlsp_country_mpa_area_score %>%
  dplyr::select(country, 
                year,
                country_mpa_area_score) %>%
  full_join(status_per_year_by_country_3, 
            by = c('country', 'year')) %>%
  rename(mpa_mgmt_score = status) %>%
  mutate(status = round((country_mpa_area_score * (mpa_mgmt_score/100))^(1/2) * 100, 1)) %>%
  group_by(country) %>%
  filter(year == max(year))
...
### 4.4 Compartive plot of Status Calculation Alternatives
TO DO ..............Ning can you plot the three alternative status scores together for each country  
