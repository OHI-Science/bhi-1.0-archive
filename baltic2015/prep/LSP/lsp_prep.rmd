...
# count number of MPAs in shape file with no matches in csv file. 
 num_mismatch_new = dplyr::count(mpa_mgmt_new, !status_csv== "NA") 
...
mpa_mgmt_with_wt = full_join( mgmt_weight, mpa_mgmt_new, 
                              by = 'mgmt_status') 
...
# weight_average = mpa_mgmt_with_wt %>%
#                  group_by(country) %>%
#                  filter(!duplicated(BSPA_ID)) %>%
#                  summarize(weight_ave = mean(weight)) %>%
#                  ungroup

r.status =full_join(ref_by_country, weight_mpa_area_by_country, by = 'country') %>% 
          full_join(weight_average, by = 'country') %>% 
          mutate(wt_mpa_area = weight * mpa_area_km2) %>%
