...
There are three types of management status and their corresponding weight: No Plan (0.3), Indevelopment (0.6), and Implemented (1).  

For some MPAs, there are multiple plans in different stages of implementation. For those MPAs, the weights have been averaged.

While combining MPA management data with MPA area data from the shapefile, 16 MPAs from the shapefile did not have a match from the management dataset. For those MPAs, the weight has been assigned the same weight as No Plan (0.3). 

``` {r combine management status with mpa area and establishment info, echo = FALSE}

mgmt_status_old = read_csv(file.path(dir_prep, 'mpa_data_database/MPA_management_plans_20160415.csv')) %>%
              dplyr::select(name_csv = helcom_mpa,
                            plan = management_plan_name, 
                            BSPA_ID = id, 
                            mgmt_status = management_plan_status, 
                            implem_date = implementation_date_of_current_management_plan)

## to check the number of unique MPAs from the 214 total observations

# library(base)
# mpa_name = dplyr::select(mgmt_status, name) 
# unique_mpa_csv = base::unique(mpa_name) ## total of 164 names that are unique

#  Combine managment status data with MPA area data

mpa_mgmt_old = right_join(mgmt_status_old, mpa_area_per_rgn,  # select only MPAs with BHI_ID
                      by = c("BSPA_ID")) %>%
               arrange(mgmt_status) %>%
               mutate(mgmt_status = ifelse(is.na(mgmt_status), as.character(mpa_status), mgmt_status),
                      date_est = str_sub(date_est, start = -4)) %>% # only select the year of establishment
               dplyr::select(-mpa_status)

# count number of MPAs in shape file with no matches in csv file. 

num_mismatch = dplyr::count(mpa_mgmt_old, !mgmt_status== "NA") # 30 no match out of 305 observations total. 

# save as csv

write_csv(mpa_mgmt_old, file.path(dir_lsp, 'mpa_mgmt_status_old_csv.csv'))

# assign weight to management status

mgmt_weight = data.frame( mgmt_status = c("No Plan", "In development", "Implemented"),
                          weight = c(0.3, 0.6, 1) )
  
mpa_mgmt_with_wt = full_join(mpa_mgmt_old, mgmt_weight) %>%
           group_by(name) %>% 
           mutate(ave_weight = mean(weight)) %>%
  


# TODO: save file to layers and register

r.status = mpa_mgmt %>%
         mutate(ref_point = 0.1 * eez_area_km2,
                wt_mpa_area = weight * mpa_area_km2) %>%
         group_by(BHI_ID) %>% 
         mutate(sum_area_wt = sum(wt_mpa_area)) %>%
         summarize(status = sum_area_wt / ref_point *100)
         

# temporary code to save csv of mpa_mgmt with NA's            
# write_csv(mpa_mgmt, file.path(dir_prep, ""))


##########  2nd try, with updated csv file - NOT working ##########

# read in new mpa mgmt status csv file, NO Plans information included in this file. 

mgmt_status_new = read.csv(file.path(dir_prep, "mpa_data_database/MPA_status_download06June2016.csv"), sep = ";") %>%
...
mpa_name = dplyr::select(mgmt_status_new, name_csv)
...
mpa_mgmt_new = right_join(mgmt_status_new, mpa_area_per_rgn,  
                          by = "BSPA_ID") 
...
write_csv(mpa_mgmt_new, file.path(dir_lsp, 'mpa_mgmt_status_new_csv.csv'))

# count number of MPAs in shape file with no matches in csv file. 
num_mismatch_new = dplyr::count(mpa_mgmt_new, !status_csv== "NA") # 13 rows no match

````

Note: status calc: use all plans, take average. Add a column to track how many plans are used for each MPA. 

``` {r status calculation, ECHO = FALSE}

mgmt_weight = data.frame( status_csv = c(NA, "Designated", "Designated_and_partly_managed", "Designated_and_managed"),
                          weight = c(0.3, 0.3, 0.6, 1) )

mpa_mgmt_with_wt = full_join(mpa_mgmt_new, mgmt_weight,
                             by = "status_csv") 
...
r.status = mpa_mgmt_with_wt %>%
         mutate(ref_point = 0.1 * eez_area_km2,
                wt_mpa_area = weight * mpa_area_km2) %>%
         group_by(country) %>% 
         mutate(status = sum(wt_mpa_area) / ref_point *100)
