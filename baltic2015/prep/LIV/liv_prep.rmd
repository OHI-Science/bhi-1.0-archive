...
```{r join regional_employ1 with nuts_pop_area1}
...
```{r Get Finnish data that was excluded}
...
```{r Join Finnish data to other regional data}
...
```{r restrict regional dataset years}
...
```{r are bhi regions missing from regional employ and pop join}
...
```{r calculate number of regionally employed }
...
```{r plot number of nuts3 employed by bhi region}
ggplot(employ_nuts4)+
...
```{r jCalculate total employed by BHI region}
...
```{r prepare regional liv object for csv}
...
```{r clean russian nat data object}
...
```{r join eu nat employ percent eu_pop3}
...
```{r join eu and russian nat employ data}
colnames(ru_employ)
...
```{r plot joined EU and russian employment data}
ggplot(nat_employ4) +
...
```{r restrict national data last year}
...
```{r plot national data by BHI region}
...
```{r prepare national object for layers}
...
```{r extract liv most recent year status}
...
```{r plot liv trend and status together}
