...
```{r clean regional data object, message = FALSE}
...
```{r clean nuts pop area data, message = FALSE}
...
```{r check fi nuts2 names, message = FALSE}
...
corrected_nuts2_new = read_csv(file.path(dir_liv, 'liv_data_database/nuts_2_rgn_id_match_udpated_9.16.csv')) 
...
```{r join regional_employ1 with nuts_pop_area1, message = FALSE}
...
```{r Join Finnish data to other regional data, message = FALSE}
...
```{r are bhi regions missing from regional employ and pop join, message = FALSE}
...
```{r jCalculate total employed by BHI region, message = FALSE}
...
```{r liv status calc, message = FALSE}
...
```{r plot liv trend and status together, message = FALSE}
