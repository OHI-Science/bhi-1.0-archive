...
## TODO @jennifergriffiths: save these documents in bhi repo so can call like this 
...
if (params$datasource != 'csv') {

## connect to the BHI database 
## run your personal mysql config script to read in passcode
## NOTE: this cannot be knit in the .Rmd. Run this manually when you need to access the database. It will: 
## 1. access the database with MySQL()
## 2. save an intermediate .csv file that will be used to knit your .Rmd so that you can use the .Rmd workflow. 
...
write.csv(data, file.path(dir_mar, 'intermediate/data_sql.csv'))

} else if (params$datasource == 'csv') {

   ## load in csv file that was saved after accessing the database. 
  data = readr::read_csv(file.path(dir_mar, 'intermediate/data_sql.csv'))
