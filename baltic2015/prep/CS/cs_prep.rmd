...
```{r, child='../../conf/goals/CS.Rmd', results='asis', echo=FALSE}
```
...
### Trial: using Carbon burial rate 
...
bhi_rgn_lookup <- read.csv(file.path(dir_prep, 'bhi_basin_country_lookup.csv'), sep = ";") %>% 
  select(basin = Subbasin, rgn_id = BHI_ID)
...
```{r}
cs_data_new <- rgdal::readOGR(dsn = path.expand(file.path(dir_cs, 'zostera_raster_raw')),
                          layer = 'Join_macrophytes')  

head(cs_data_new@data)
plot(cs_data_new, col = cs_data_new@data$Z_marina)
