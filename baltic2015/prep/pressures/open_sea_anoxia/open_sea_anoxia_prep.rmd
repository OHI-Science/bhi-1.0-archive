...
for (anox_file in anox_file_list) {
  ...
  dplyr::select(x_utm, y_utm, O2_bottom)
  # head(anox) 
  ...
  # O2_bottom = raster(t, layer = 'O2_bottom')
...
bhi_no_gof = bhi%>%subset(!Subbasin == "Gulf of Finland")%>%spTransform(CRS("+init=epsg:32634 +proj=utm +zone=34 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))

# create bathymetry mask: remove cells with depth higher than -70 m

halocline_70 = bathymetry # to not overwrite the original data
halocline_70[halocline_70 > -70] = NA
t1 = projectRaster( t, halocline_70) # change t projection to laea

halocline_70 = projectRaster(halocline_70, t) # transform to the same projection as O2 data
plot(halocline_70)

#Mask
#not sure this mask is needed - JA
#m <- mask(halocline_70,bhi_no_gof)

# Loop for each O2_bottom tif file
...
## Calcualte the total area within each non-GoG BHI region

# since we know cell resolution of O2_below_70 is 1000 x 1000 m, we can just count number of cells per region and sum

rgn_ras <- bhi%>%
            spTransform(CRS("+init=epsg:32634 +proj=utm +zone=34 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 "))%>%
              rasterize(O2_bottom,progress='text', field = 'BHI_ID')
...
area_rgn <- zonal(O2_below_70, rgn_ras, fun='count')


#### additional options: 
# raster::area? 
#### raster of regions, crosstab 
# if polygon, use raster::extract -> 
## 1. raster extract O2 against polygon using the same projection (laea), count number of cells of hypoxia (casey's tutorial on vector, take the list and turn it into data frame)

## 2. create area raster in the same projection as O2 raster, if not equal area
## 3. raster extract of area raster 
## --> list from first step and third step, which should have same num of list elements, one for each region. 

# O2_below_70 = crop(O2_below_0, halocline_70) 
