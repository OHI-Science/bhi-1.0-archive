...
BP_current_anoxic_area = BP_anoxia_all_yr %>%
  ...
  dplyr::rename(current_area = area_km2) %>% 
  ungroup()
...
anoxia_pressure = anoxia_all_rgns %>% 
...
anoxia_status_all_years = anoxia_all_rgns %>% 
  dplyr::select(rgn_id, year, pressure_score) %>% 
  mutate(score = (1 - pressure_score) *100)
...
anoxia_trend = anoxia_status_all_years %>% 
  ...
  summarize(rgn_id = rgn_id, 
            score = round(coef(lmd)['year']*0.05, 2)) %>% 
