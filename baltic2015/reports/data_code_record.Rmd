...
- BHI github data
  ...
  - Data used in csv file format in github are whenever possible read in from the database.  This occurs in a script in each prep folder called “mygoal_prep_database_call.r”.  This script requires running your personal access to the database.  Then it extracts data from the database and saves in a folder in the same prep folder titled “mygoal_data_database”

  - Data are then read into the .rmd prep file from this “mygoal_data_database” folder for data layer preparation.  

  - This does not occur if data files are not yet included in the database.  In most cases, the folder “mygoal_data_database” has been created for the goal but files are manually copied in rather than sourced from the database by the script.  If this is later updated, users should check columns and column names carefully to see if the .rmd script needs to be modified.
