...
PlotFlowerMulti <- function(scores          = read.csv('scores.csv'), # dataframe with regions, goals, dimensions, scores
  ...
  # DEBUG: scores=read.csv('scores.csv'); rgn_names = read.csv('layers/rgn_global_gl2014.csv'); fld_value_id = 'region_id'; fld_value_score = 'score';dim_choice = 'score'; print_fig = TRUE; save_fig = TRUE; path_figures = 'reports/figures'; scale_label = 'score';  scale_limits = c(0, 100);overwrite = TRUE; color_scheme = 'new'
