...
Scores by region for the Baltic Sea, on three different spatial scales. BHI regions (a) are the 42 areas resulting from the intesection of the exclusive economic zones (EEZ)s of 9 Baltic countries (b) with the 17 Baltic subbasins (c). 
(*these will be arranged as subplots*)
...
ggplot(data = scores %>% filter(region_id > 300 & region_id < 500), 
       aes(x = fct_rev(goal), y = status, color = str_replace(goal_group_color, "\n", ""))) + 
  geom_point(size = 2) + 
...
write_csv(resil, "reports/manuscript/resil_mtx.csv")
